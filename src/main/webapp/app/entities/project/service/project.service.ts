...
type RestOf<T extends IProject | NewProject> = Omit<T, 'creationDate'> & {
...
export class ProjectService {
  ...
  protected convertDateFromClient<T extends IProject | NewProject | PartialUpdateProject>(project: T): RestOf<T> {
    return {
      ...project,
      creationDate: project.creationDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProject: RestProject): IProject {
    return {
      ...restProject,
      creationDate: restProject.creationDate ? dayjs(restProject.creationDate) : undefined,
    };
  }
