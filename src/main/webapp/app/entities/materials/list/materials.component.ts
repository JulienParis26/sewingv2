...
import { EntityArrayResponseType, MaterialsService } from '../service/materials.service';
...
export class MaterialsComponent implements OnInit {
  ...
  trackId = (_index: number, item: IMaterials): number => this.materialsService.getMaterialsIdentifier(item);
  ...
  protected refineData(data: IMaterials[]): IMaterials[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IMaterials[] | null): IMaterials[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.materialsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
