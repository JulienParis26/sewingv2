...
public class ProjectRepositoryWithBagRelationshipsImpl implements ProjectRepositoryWithBagRelationships {
    ...
    public Optional<Project> fetchBagRelationships(Optional<Project> project) {
        return project.map(this::fetchFabrics);
    }
    ...
    public List<Project> fetchBagRelationships(List<Project> projects) {
        return Optional.of(projects).map(this::fetchFabrics).orElse(Collections.emptyList());
    }

    Project fetchFabrics(Project result) {
        return entityManager
            .createQuery("select project from Project project left join fetch project.fabrics where project is :project", Project.class)
            .setParameter("project", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Project> fetchFabrics(List<Project> projects) {
        ...
        List<Project> result = entityManager
            .createQuery(
                "select distinct project from Project project left join fetch project.fabrics where project in :projects",
                Project.class
            )
